...
export default function AddItem() {
  ...
  useEffect(() => {
    const fetchIngredients = async () => {
      try {
        ...
        setValidIngredients([]);
        setError(
          err.response?.data?.message || 'Failed to load ingredients. Please try again.'
        );
      }
    };
    fetchIngredients();
  ...
  const handleNewIngredientChange = (e) => {
    const { name, value } = e.target;
    const updatedValue = name === 'volume' ? parseFloat(value) || '' : value;

    setNewIngredient((prev) => ({ ...prev, [name]: updatedValue }));
    setError(null);
  ...
  const addIngredient = () => {
    ...
    const ingredientToAdd = {
      ...
      volume: parseFloat(newIngredient.volume) || 0,
    };
    ...
    if (ingredientIdRef.current) {
      ingredientIdRef.current.focus();
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/item/create', {
        ...formData,
